...
def handler(request):
    ...
    try:
        ...
        if path == '/api/train-xgboost':
            data = request.get_json()
            csv_data = data.get('training_data', [])
            
            if not csv_data:
                return jsonify({'status': 'error', 'message': 'No training data provided'}), 400, headers
            ...
            return jsonify({'status': 'error', 'message': 'Endpoint not found'}), 404, headers
